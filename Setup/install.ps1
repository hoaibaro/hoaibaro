...
$Global:EthernetDriverExe = $Global:config.sourcePaths.ethernetDriver
$Global:WifiDriverExe = $Global:config.sourcePaths.wifiDriver
...
function Install-DriverExe {
    ...
    try {
        ...
        $installed = $false
        foreach ($arguments in $commonArgs) {
            try {
                ...
                else { Add-Status "$Type driver installer returned code $($p.ExitCode) with args: $arguments" $statusTextBox ([System.Drawing.Color]::Yellow) }
            }
            catch { Add-Status "$Type driver install attempt failed with args '$arguments': $_" $statusTextBox ([System.Drawing.Color]::Yellow) }
        }
        ...
        Add-Status "Waiting for $Type adapter to initialize..." $statusTextBox
        $adapterFound = $false
        $maxWaitSeconds = 30
        $waitTime = 0
        while ($waitTime -lt $maxWaitSeconds) {
            $adapter = $null
            if ($Type -eq 'WiFi') {
                $adapter = Get-NetAdapter | Where-Object { $_.InterfaceDescription -match "(?i)(wireless|wifi|802\.11|wi-fi|wlan|wi\\s?fi|ax200|ax201|ax210|ac\\d{4})" -or $_.InterfaceType -eq 71 }
            } else { # Ethernet
                $adapter = Get-NetAdapter -Physical | Where-Object { $_.InterfaceType -ne 71 -and $_.Status -ne 'Disconnected' }
            }

            if ($adapter) {
                Add-Status "$Type adapter detected." $statusTextBox ([System.Drawing.Color]::Green)
                $adapterFound = $true
                break
            }
        ...
        if (-not $adapterFound) {
            Add-Status "Timed out waiting for $Type adapter. Continuing anyway..." $statusTextBox ([System.Drawing.Color]::Yellow)
        }
...
function Install-WiFiDriversOffline {
    ...
    try {
        ...
        $driverPath = Join-Path $Global:config.sourcePaths.drivers "WIFI"
        $candidatePaths = @($driverPath) | Where-Object { Test-Path $_ }

        $selectedPath = $null
        if ($candidatePaths.Count -gt 0) {
            $selectedPath = $candidatePaths[0]
            Add-Status "Using driver path: $selectedPath" $statusTextBox
        }
        else {
        ...
        # Install all INF recursively
        Add-Status "Installing drivers from: $selectedPath" $statusTextBox
        $pnputil = "pnputil.exe"
        $arguments = @("/add-driver", ('"{0}\*.inf"' -f $selectedPath), "/subdirs", "/install")
        Start-Process -FilePath $pnputil -ArgumentList $arguments -NoNewWindow -PassThru -Wait
...
function Invoke-WiFiAutoConnection {
    ...
    try {
        ...
        $targetSSID = $Global:config.wifi.ssid
        ...
        # Create WiFi profile
        $SSID = $Global:config.wifi.ssid
        $Password = $Global:config.wifi.password
        $profileFile = "$env:TEMP\$($SSID)_profile.xml"
...
function Invoke-WindowsUpdateCheck {
    ...
    try {
        ...
        try {
            # Use USOClient for immediate trigger, fully async
            Add-Status "Triggering Windows Update scan in background..." $statusTextBox
            Start-Process -FilePath "USOClient.exe" -ArgumentList "StartScan" -WindowStyle Hidden -ErrorAction Stop
            # The other commands can be triggered as well to queue up the actions
...
function Invoke-RenamebyDevice {
    ...
    try {
        ...
        Add-Status "Current computer name: $currentName" $statusTextBox ([System.Drawing.Color]::Gray)
        ...
        # Prefix
        $prefix = ""
        if ($deviceType -eq "Desktop") { $prefix = $Global:config.computerNamePrefixes.desktop }
        elseif ($deviceType -eq "Laptop") { $prefix = $Global:config.computerNamePrefixes.laptop }
...
function Invoke-StartupOptimization {
    param ([System.Windows.Forms.RichTextBox]$statusTextBox)
    $startupPrograms = $Global:config.startupProgramsToRemove
...
function Invoke-TimezoneConfiguration {
    param ([System.Windows.Forms.RichTextBox]$statusTextBox)
    try {
        $tzResult = Start-Process -FilePath "tzutil" -ArgumentList "/s `"$($Global:config.timezone)`"" -Wait -PassThru -WindowStyle Hidden
        if ($tzResult.ExitCode -eq 0) {
            Add-Status "Time zone set to $($Global:config.timezone) completed." $statusTextBox ([System.Drawing.Color]::Gray)
        }
...
function Invoke-PowerOptionsConfiguration {
    ...
    try {
        ...
        Add-Status "Power options configured to 'Do Nothing' completed." $statusTextBox ([System.Drawing.Color]::Gray)
    }
    catch {
...
function Invoke-ActivateConfiguration {
    param ([string]$deviceType, [System.Windows.Forms.RichTextBox]$statusTextBox)
    try {
        # --- 1. Windows 11 Pro Activation ---
        Invoke-ActivateWindows11Pro $statusTextBox
...
function Invoke-RunAllOperations {param([System.Windows.Forms.Form]$mainForm)
    ...
    try {
        ...
        if ($updateResult) {
            Add-Status "Windows Update process started." $statusTextBox ([System.Drawing.Color]::Gray)
        }
        else {
...
# [2] Install Software Functions (Refactored)
...
function Test-OneDriveInstalled {
    ...
    foreach ($keyPath in $uninstallKeys) {
        try {
            ...
            if ($statusTextBox) { Add-Status "ERROR: OneDrive installation check failed: $_" $statusTextBox ([System.Drawing.Color]::Red) }
        }
    }
    ...
    try {
        ...
        if ($statusTextBox) {
            Add-Status "ERROR: OneDrive installation check via Get-Package failed: $($_.Exception.Message)" $statusTextBox ([System.Drawing.Color]::Red)
        }
    }
    return $false
}
...
function Uninstall-OneDriveComplete {
    ...
    try {
        if (-not (Test-OneDriveInstalled -statusTextBox $statusTextBox)) {
            Add-Status "OneDrive: Not found, skipping uninstall." $statusTextBox
            return $true
        }

        Add-Status "OneDrive: Detected. Starting full uninstallation..." $statusTextBox
        $oneDriveProcesses = "OneDrive", "OneDriveSetup"
        Get-Process -Name $oneDriveProcesses -ErrorAction SilentlyContinue | Stop-Process -Force

        $setupPaths = @(
            "$env:SYSTEMROOT\SysWOW64\OneDriveSetup.exe",
            "$env:SYSTEMROOT\System32\OneDriveSetup.exe",
            ([System.Environment]::GetFolderPath('localapplicationdata') + '\Microsoft\OneDrive\OneDriveSetup.exe')
        )
        $uninstalled = $false
        foreach ($setupPath in $setupPaths) {
            if (Test-Path $setupPath) {
                Add-Status "  -> Running $setupPath /uninstall..." $statusTextBox
                $p = Start-Process -FilePath $setupPath -ArgumentList "/uninstall /allusers /silent /force" -Wait -PassThru
        ...
        if ($uninstalled) {
            Add-Status "OneDrive: Uninstalled successfully." $statusTextBox ([System.Drawing.Color]::Green)
        } else {
            Add-Status "OneDrive: Main uninstall failed. Trying fallback..." $statusTextBox ([System.Drawing.Color]::Yellow)
        }
        
        # Force-remove remaining files and folders
        $leftoverPaths = @(
            ([System.Environment]::GetFolderPath('localapplicationdata') + '\Microsoft\OneDrive'),
            ([System.Environment]::GetFolderPath('programdata') + '\Microsoft\OneDrive'),
            "C:\OneDriveTemp"
        )
        ...
        if (-not (Test-OneDriveInstalled -statusTextBox $statusTextBox)) {
            Add-Status "OneDrive: Cleanup complete." $statusTextBox ([System.Drawing.Color]::Green)
            return $true
        } else {
            Add-Status "OneDrive: Still detected after cleanup. Manual removal may be needed." $statusTextBox ([System.Drawing.Color]::Red)
            return $false
        }
    }
    catch {
...
function Invoke-InstallSoftware {
    ...
    $softwarePackages = $Global:config.software
    if (-not $softwarePackages) {
        Add-Status "FATAL: 'software' section not found in config.json." $statusTextBox ([System.Drawing.Color]::Red)
        return $false
    }

    $baseSourceDir = $Global:config.sourcePaths.software
    $tempSetupDir = "$env:USERPROFILE\Downloads\SETUP"

    try {
        # --- 1. Create Temp Directory ---
        if (-not (Test-Path $tempSetupDir)) {
            New-Item -Path $tempSetupDir -ItemType Directory -Force | Out-Null
        }

        # --- 2. Pre-copy shared files (Unikey, Agents, etc. - Preserved Logic) ---
        Add-Status "Copying shared utilities..." $statusTextBox
        # ... (Copy logic for Unikey, SC-*, Agents, ManageEngine)
        # This part is preserved and adapted from the old function
        $destCopy = "$env:USERPROFILE\Downloads"
        $unikeyFolder = Get-ChildItem -Path (Join-Path $baseSourceDir 'unikey*') -Directory -ErrorAction SilentlyContinue | Select -First 1
        if ($unikeyFolder) {
            ...
            if (-not (Test-Path $unikeyDest)) {
                ...
                Add-Status "  -> Unikey copied." $statusTextBox
            } else { Add-Status "  -> Unikey already exists. Skipped." $statusTextBox }
        }

        if ($DeviceType -eq "Desktop") {
            $desktopAgent = Get-ChildItem -Path (Join-Path $baseSourceDir 'Desktop Agent*.exe') -File -ErrorAction SilentlyContinue | Select -First 1
            if ($desktopAgent) {
                Copy-Item -Path $desktopAgent.FullName -Destination $destCopy -Force
                Add-Status "  -> Desktop Agent copied." $statusTextBox
            }
        } else { # Laptop
            $laptopAgent = Get-ChildItem -Path (Join-Path $baseSourceDir 'Laptop Agent*.exe') -File -ErrorAction SilentlyContinue | Select -First 1
            if ($laptopAgent) {
                Copy-Item -Path $laptopAgent.FullName -Destination $destCopy -Force
                Add-Status "  -> Laptop Agent copied." $statusTextBox
            }
            $mdmSource = Join-Path $baseSourceDir "ManageEngine_MDMLaptopEnrollment"
            $mdmDest = Join-Path $destCopy "ManageEngine_MDMLaptopEnrollment"
            if (-not (Test-Path $mdmDest)) {
                robocopy $mdmSource $mdmDest /E /NFL /NDL /NJH /NJS /nc /ns /np /R:2 /W:5 | Out-Null
                Add-Status "  -> ManageEngine MDM files copied." $statusTextBox
            } else { Add-Status "  -> ManageEngine MDM files already exist. Skipped." $statusTextBox }
        }

        # --- 3. Process all software packages from config.json ---
        foreach ($app in $softwarePackages) {
            if (-not $app.enabled) { continue }
            if ($app.device -ne 'All' -and $app.device -ne $DeviceType) { continue }

            Add-Status "Processing: $($app.displayName)" $statusTextBox

            # A. Handle Uninstall Tasks
            if ($app.task -eq 'uninstall') {
                $isInstalledCheck = & ([ScriptBlock]::Create($app.checkFunction)) -statusTextBox $statusTextBox
                if ($isInstalledCheck) {
                    Add-Status "  -> Uninstalling..." $statusTextBox
                    $uninstallResult = & ([ScriptBlock]::Create($app.uninstallFunction)) -statusTextBox $statusTextBox
                    if ($uninstallResult) { Add-Status "  -> Uninstalled successfully." $statusTextBox ([System.Drawing.Color]::Green) }
                    else { Add-Status "  -> Uninstallation failed." $statusTextBox ([System.Drawing.Color]::Yellow) }
                } else { Add-Status "  -> Not installed. Skipping uninstall." $statusTextBox }
                continue
            }

            # B. Handle Install Tasks
            $isInstalled = $false
            if ($app.checkType -eq 'path') {
                foreach ($path in $app.checkPaths) {
                    $expandedPath = $ExecutionContext.InvokeCommand.ExpandString($path)
                    if (Test-Path $expandedPath) { $isInstalled = $true; break }
                }
            } elseif ($app.checkType -eq 'function') {
                $checkResult = & ([ScriptBlock]::Create($app.checkFunction))
                if (($checkResult -is [boolean] -and $checkResult) -or ($checkResult -is [hashtable] -and $checkResult.Installed)) {
                    $isInstalled = $true
                }
            }

            if ($isInstalled) { Add-Status "  -> Already installed. Skipping." $statusTextBox; continue }

            Add-Status "  -> Queued for installation." $statusTextBox ([System.Drawing.Color]::Yellow)
            
            # Copy installer
            $sourceDir = if ($app.sourceSubDir) { Join-Path $baseSourceDir $app.sourceSubDir } else { $baseSourceDir }
            $installerFile = Get-ChildItem -Path $sourceDir -Filter $app.installerName -File -ErrorAction SilentlyContinue | Select-Object -First 1
            
            if (-not $installerFile) {
                Add-Status "  -> ERROR: Installer '$($app.installerName)' not found in '$sourceDir'." $statusTextBox ([System.Drawing.Color]::Red)
                continue
            }

            $destPath = ""
            if ($app.installerType -eq 'office') {
                $destPath = Join-Path $tempSetupDir $app.sourceSubDir
                Add-Status "  -> Copying Office source..." $statusTextBox
                robocopy $sourceDir $destPath /E /NFL /NDL /NJH /NJS /nc /ns /np /R:2 /W:5 | Out-Null
            } else {
                $destPath = Join-Path $tempSetupDir $installerFile.Name
                Copy-Item -Path $installerFile.FullName -Destination $destPath -Force
            }

            # Install
            Add-Status "  -> Installing..." $statusTextBox
            try {
                $process = $null
                if ($app.installerType -eq 'msi') {
                    $finalArgs = $app.installArgs.Replace('{installerPath}', "`"$destPath`"")
                    $process = Start-Process -FilePath "msiexec.exe" -ArgumentList $finalArgs -Wait -PassThru -WindowStyle Hidden
                } elseif ($app.installerType -eq 'exe') {
                    $process = Start-Process -FilePath $destPath -ArgumentList $app.installArgs -Wait -PassThru -WindowStyle Hidden
                } elseif ($app.installerType -eq 'office') {
                    $officeSetupExe = Join-Path $destPath "setup.exe"
                    $configFile = Join-Path $destPath "config.xml"
                    if (-not (Test-Path $configFile)) {
                        $configContent = @"
<Configuration>
  <Add OfficeClientEdition="64" Channel="PerpetualVL2019">
    <Product ID="ProPlus2019Volume" PIDKEY="$($Global:config.officeActivation.productKey)">
      <Language ID="en-us" />
      <ExcludeApp ID="Access" /><ExcludeApp ID="Groove" /><ExcludeApp ID="Lync" /><ExcludeApp ID="OneNote" /><ExcludeApp ID="Publisher" />
    </Product>
  </Add>
...
"@
                    ...
                    $process = Start-Process -FilePath $officeSetupExe -ArgumentList "/configure `"$configFile`"" -Wait -PassThru -NoNewWindow
                }

                if ($process -and ($process.ExitCode -in 0, 3010)) {
                    Add-Status "  -> Installation successful (Code: $($process.ExitCode))." $statusTextBox ([System.Drawing.Color]::Green)
                } else {
                    Add-Status "  -> ERROR: Installer failed with code: $($process.ExitCode)." $statusTextBox ([System.Drawing.Color]::Red)
                }
            } catch {
                Add-Status "  -> FATAL ERROR during installation: $($_.Exception.Message)" $statusTextBox ([System.Drawing.Color]::Red)
            }
        }
        return $true
    }
    catch {
        Add-Status "FATAL ERROR in provisioning engine: $($_.Exception.Message)" $statusTextBox ([System.Drawing.Color]::Red)
        return $false
    }
    finally {
        if (Test-Path $tempSetupDir) {
            Add-Status "--- Cleaning up temporary setup files... ---" $statusTextBox
            Remove-Item -Path $tempSetupDir -Recurse -Force -ErrorAction SilentlyContinue
        }
    }
}
...
function Invoke-SetTimezonePower {
    param([System.Windows.Forms.RichTextBox]$statusTextBox)
    try {
        Add-Status "Setting time zone to $($Global:config.timezone)..." $statusTextBox ([System.Drawing.Color]::Gray)

        # Set timezone to SE Asia Standard Time
        Start-Process -FilePath "tzutil.exe" -ArgumentList "/s `"$($Global:config.timezone)`"" -Wait -NoNewWindow
        ...
        Add-Status "Time zone, power options completed." $statusTextBox ([System.Drawing.Color]::Gray)
    }
    catch {
        Add-Status "Error: $_" $statusTextBox ([System.Drawing.Color]::Red)
    }
...
function Invoke-FirewallOn {
    param([System.Windows.Forms.RichTextBox]$statusTextBox)
    try {
        Add-Status "Turning on the firewall..." $statusTextBox ([System.Drawing.Color]::Gray)
        ...
        Add-Status "Firewall has been turned on." $statusTextBox ([System.Drawing.Color]::Gray)
    }
    catch {
        Add-Status "Error: $_" $statusTextBox ([System.Drawing.Color]::Red)
    }
...
function Invoke-FirewallOff {
    param([System.Windows.Forms.RichTextBox]$statusTextBox)
    try {
        Add-Status "Turning off the firewall..." $statusTextBox ([System.Drawing.Color]::Gray)
        ...
        Add-Status "Firewall has been turned off." $statusTextBox ([System.Drawing.Color]::Gray)
    }
    catch {
        Add-Status "Error: $_" $statusTextBox ([System.Drawing.Color]::Red)
    }
...
"@
    ...
    $btnRenameVolume = New-DynamicButton -text "Rename Volume" -x 330 -y 150 -width 150 -height 40 -normalColor ([System.Drawing.Color]::FromArgb(0, 150, 0)) -hoverColor ([System.Drawing.Color]::FromArgb(0, 200, 0)) -pressColor ([System.Drawing.Color]::FromArgb(0, 100, 0)) -clickAction {
        ...
        Add-Status "Ready to rename volume. Select a drive, enter a new label, click Rename Volume." $statusTextBox
        Update-DriveList $driveListBox
    }
...
function Invoke-ActivateWindows11Pro {
    param([System.Windows.Forms.RichTextBox]$statusTextBox)
    try {
        # Display current Windows version
        $currentWindowsVersion = Get-WindowsVersionShort
        Add-Status "Checking Activation Status of Windows..." $statusTextBox ([System.Drawing.Color]::Gray)
        Add-Status "OS: $currentWindowsVersion" $statusTextBox ([System.Drawing.Color]::Gray)
        ...
        Add-Status "Windows not activated. Activating Windows 11 Pro..." $statusTextBox
...
function Invoke-ActivateOffice2019 {
    param([System.Windows.Forms.RichTextBox]$statusTextBox)
    try {
        Add-Status "Checking Activation Status of Office..." $statusTextBox ([System.Drawing.Color]::Gray)
        ...
        if (-not $officePath) {
            Add-Status "Office not found. Please install." $statusTextBox ([System.Drawing.Color]::Yellow)
            return
        }
        ...
        try {
            ...
            if ($isActivated) {
                Add-Status "Office activated." $statusTextBox ([System.Drawing.Color]::Gray)
                return
            }
        }
        catch {
            Add-Status "Warning: Could not check activation status: $_" $statusTextBox ([System.Drawing.Color]::Yellow)
        }
        ...
        try {
            ...
            if ($activateResult -match "successful" -or $activateResult -match "activated") {
                Add-Status "Office 2019 Pro Plus activated." $statusTextBox
            }
            else {
...
function Invoke-ActivationDialog {
    ...
    $btnWin10Pro = New-DynamicButton -text "Windows Pro" -x 10 -y 50 -width 235 -height 40 -normalColor ([System.Drawing.Color]::FromArgb(0, 150, 0)) -hoverColor ([System.Drawing.Color]::FromArgb(0, 200, 0)) -pressColor ([System.Drawing.Color]::FromArgb(0, 100, 0)) -clickAction {
        Invoke-ActivateWindows11Pro -statusTextBox $statusTextBox
    }
        ...
        try {
            ...
            if ($currentFeature) {
                ...
                if ($currentState -eq "Enabled") {
                    Add-Status "$($feature.DisplayName): Already enabled. Skipping..." $statusTextBox ([System.Drawing.Color]::Gray)
                }
                elseif ($currentState -eq "Disabled") {
                    Add-Status "$($feature.DisplayName): Currently disabled. Enabling..." $statusTextBox ([System.Drawing.Color]::Gray)
                    ...
                    if ($enableResult.ExitCode -eq 0) {
                        Add-Status "$($feature.DisplayName): Enabled completed." $statusTextBox ([System.Drawing.Color]::Gray)
                    }
                    elseif ($enableResult.ExitCode -eq 3010) {
                        Add-Status "$($feature.DisplayName): Enabled completed. (Restart required)" $statusTextBox ([System.Drawing.Color]::Gray)
                    }
                    else {
...
function Invoke-DisableWindowsFeatures {
    ...
    foreach ($feature in $featuresToDisable) {
        ...
        if ($feature.SupportedOS -and -not ($osVersion -like "*$($feature.SupportedOS)*")) {
            Add-Status "$($feature.DisplayName): Not apply on $osVersion." $statusTextBox([System.Drawing.Color]::Gray)
            continue
        }
        try {
            ...
            if ($currentFeature) {
                ...
                if ($currentState -eq "Disabled") {
                    Add-Status "$($feature.DisplayName): Already disabled." $statusTextBox([System.Drawing.Color]::Gray)
                }
                elseif ($currentState -eq "Enabled") {
                    Add-Status "$($feature.DisplayName): Currently enabled. Disabling..." $statusTextBox([System.Drawing.Color]::Gray)
                    ...
                    if ($disableResult.ExitCode -eq 0) {
                        Add-Status "$($feature.DisplayName): Disabled completed." $statusTextBox([System.Drawing.Color]::Gray)
                    }
                    elseif ($disableResult.ExitCode -eq 3010) {
                        Add-Status "$($feature.DisplayName): Disabled completed. (Restart required)" $statusTextBox([System.Drawing.Color]::Gray)
                    }
                    else {
                    ...
                    if ($newFeature) {
                        Add-Status "$($feature.DisplayName): Verified new state is $($newFeature.State)" $statusTextBox([System.Drawing.Color]::Gray)
                    }
                }
                else {
                    Add-Status "WARNING: $($feature.DisplayName) is in unexpected state: $currentState" $statusTextBox([System.Drawing.Color]::Yellow)
