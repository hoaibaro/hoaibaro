...
function Invoke-RenameDialog {
    ...
    $renameButton.Add_Click({
            ...
            # Disable button ð? tránh click nhi?u l?n
            $renameButton.Enabled = $false

            # Clear status trý?c khi b?t ð?u
            $statusTextBox.Clear()
            ...
            if ($confirmResult -eq [System.Windows.Forms.DialogResult]::Yes) {
                try {
                    ...
                    # H?i user có mu?n restart ngay không
...
function Set-UserPassword {
    ...
    try {
        # Ki?m tra xem ngý?i dùng có ðang dùng Windows Hello PIN không
        $pinEnabled = $false
        try {
        ...
        # N?u ðang dùng PIN, vô hi?u hóa t?m th?i
        if ($pinEnabled) {
            try {
                # Lýu tr?ng thái PIN hi?n t?i
        ...
        # N?u không dùng PIN ho?c x? l? PIN th?t b?i, dùng phýõng pháp thông thý?ng
        $methods = @(
            {
                # Phýõng pháp 1: S? d?ng LocalAccount module
                try {
